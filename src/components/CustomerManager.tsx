...
import { CustomerUser, WalletTransaction } from '../types';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-4 hover:border-amber-500/30 transition-colors">
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium">مجموع امتیازات</span>
            <Star className="w-4 h-4 text-amber-400" />
          </div>
          <div>
            <span className="text-xl sm:text-2xl font-bold text-white">{toPersianDigits(totalRewardPoints)}</span>
            <span className="text-xs text-slate-400 mr-1">امتیاز</span>
          </div>
        </div>
      ...
      {filteredCustomers.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center text-slate-400 space-y-3">
          ...
          <p className="text-xs">مشتریان پس از اولین خرید در ربات تلگرام به صورت خودکار اضافه می‌شوند.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-800">
                <div className="flex items-center gap-1.5 text-[11px] text-slate-500">
                  ...
                  <span>آخرین فعالیت: {formatDatePersian(customer.lastActiveAt)}</span>
                </div>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
